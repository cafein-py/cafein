...
def trip_distances(gtfs_paths, include=None):
    ...
    Returns
    -------
    list of (str, list of float, str)
        ``(trip_id, cumulative_meters, provenance)`` rows suitable for
    ...
    for feed, path in enumerate(gtfs_paths):
        if include is None:
            ...
            local = set(include)
        for trip_id, cumulative, tier in _feed_trip_distances(path, local):
...
def _feed_trip_distances(path, include=None):
    """The ladder over one feed, one result row per included trip."""
    ...
    for index, trip_id in enumerate(trip_ids):
        ...
        if key not in cache:
            cache[key] = _ladder(
                shape_id, route_type, stop_ids, dist_arrays[index], coordinates, shapes
            )
        cumulative, tier = cache[key]
        results.append((trip_id, cumulative, tier))
    return results


def _ladder(shape_id, route_type, stop_ids, raw_distances, coordinates, shapes):
    ...
    if cumulative is not None:
        return cumulative.tolist(), SHAPE_DIST
    if shape_id is not None and shapes() is not None:
        lines, transformer = shapes()
        if shape_id in lines:
            cumulative = _linear_referenced(lines[shape_id], latlon, transformer)
            if cumulative is not None:
                return cumulative.tolist(), SHAPE_LINREF
    return (crow * _detour(route_type)).tolist(), CROW_FLY
...
def _linear_referenced(line, latlon, transformer):
    ...
    The shape must be denser than the stop sequence (otherwise it is
    stop-to-stop straight lines dressed up as a shape), every stop must
    lie near it, and the referenced positions must be monotone (guards
    against self-intersecting shapes assigning stops out of order).
    """
...
def _shape_lines(path, stops):
    ...
    try:
        ...
        for shape_id, group in ordered.groupby("shape_id", sort=False):
            if len(group) >= 2:
                lines[shape_id] = shapely.LineString(
                    np.column_stack([group["x"], group["y"]])
                )
