...
def test_geometry_output_is_controllable(tmp_path):
    ...
    transit = lean.route_between_stops("S1", "S2", "2022-02-22 07:30:00")[0]["legs"][1]
...
def test_a_departure_window_profiles_the_k_trains(network):
    ...
    journeys = network.route_between_stops(
        "4810551", "1250551", "2022-02-22 08:30:00", departure_time_window=30
    )
...
def test_no_service_on_a_date_outside_the_feed_window(network):
    journeys = network.route_between_stops("4810551", "1250551", "2022-06-01 08:30:00")
    assert journeys == []
...
def test_access_stops_respect_the_walking_cutoff(network_with_footpaths):
    lat, lon = stop_coordinates(network_with_footpaths, "1040602")
    walkable = network_with_footpaths.access_stops(lat, lon, max_walking_time=10.0)
    ...
    nearby = network_with_footpaths.access_stops(lat, lon, max_walking_time=2.0)
...
def test_door_to_door_window_profiles_departures(network_with_footpaths):
    ...
    profile = network_with_footpaths.route_between_coordinates(
        origin, destination, "2022-02-22 08:30:00", departure_time_window=10
    )
...
def test_travel_times_from_coordinate_seed_walkable_stops(network_with_footpaths):
    ...
    reached = network_with_footpaths.travel_times_from_coordinate(
        origin, "2022-02-22 08:30:00", max_walking_time=10.0
    )
    walkable = network_with_footpaths.access_stops(*origin, max_walking_time=10.0)
...
def test_max_rides_counts_boardings(network):
    # The default max_rides=8 is the old core cap of 7 transfers: the
    # public form and the seconds-space twin return identical journeys.
    default = network.route_between_stops("4810551", "1250551", "2022-02-22 08:30:00")
    explicit = network._route_between_stops(
        "4810551", "1250551", "2022-02-22", "08:30:00", max_transfers=7
    )
    assert default == explicit
    # A one-ride cap admits single-boarding journeys only.
    single = network.route_between_stops(
        "4810551", "1250551", "2022-02-22 08:30:00", max_rides=1
    )
